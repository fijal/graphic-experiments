...
class Triangle(object):
    ...
    def enum_points(self, precision):
        v1 = self.b - self.a
        v2 = self.c - self.a
        for j in range(precision + 1):
            for i in range(precision + 1 - j):
                yield self.a + v1 * (j / precision) + v2 * (i / precision)
...
class Sphere(object):
    ...
    def enum_points(self, precision):
        ratio = math.pi / precision
        for beta in range(-precision, precision + 1):
            bsin = math.sin(beta * ratio)
            bcos = math.cos(beta * ratio)
            for alpha in range(0, 2 * precision):
                asin = math.sin(alpha * ratio)
                acos = math.cos(alpha * ratio)
                yield self.center + self.r * (
                    Vec3(asin * bcos, acos * bcos, bsin))
